...
namespace LowoUN.Module.UI
{
    ...
    public class UIManager : SingletonSimple<UIManager>
    {
        ...
        public Transform ParentLayer_Above3D => above3DLayer;
        // 一般是场景的默认hud界面
        public Transform ParentLayer_SceneHud => hudLayer;
        // HUD界面的动态补充
        public Transform ParentLayer_AddOnHud => floorLayer;
        // 二级界面
        public Transform ParentLayer_Popup => midLayer;
        // 业务级 通用提示框，比如 通用领取奖励的框，必须层级在所有二级界面之上（不同于 全局通用事件提示的ui，比如 floatTip, waiting check_dlg）
        public Transform ParentLayer_Tip => tipLayer;
        // 全局 通用提示框，比如 服务器提示下线，Check_Dlg
        public Transform ParentLayer_GlobalNotice => topLayer;

        void SetHudUI_Show()
        {
            above3DLayer.gameObject.SetActive(true);
            hudLayer.gameObject.SetActive(true);
            floorLayer.gameObject.SetActive(true);
        }
        void SetHudUI_Hide()
        {
            above3DLayer.gameObject.SetActive(false);
            hudLayer.gameObject.SetActive(false);
            floorLayer.gameObject.SetActive(false);
        }
        ...
        public void Init(Action cb_init)
        {
            ...
            Debug.Log("UIRootView Init Succ");

            // if (tipManager == null)
            //     tipManager = new UITipManager();

            topLayer = _view.TopLayer;
            midLayer = _view.MidLayer;
            tipLayer = _view.TipLayer;
            floorLayer = _view.FloorLayer;
            hudLayer = _view.HUDRoot;
            above3DLayer = _view.Above3D;
        ...
        public void ClearSceneAll()
        {
            ...
            foreach (Transform child in tipLayer)
            {
                Log.Print($"HandleUI RmvUI ClearSceneAll() ui tipLayer ___ name:{child.name}, child GetType().Name:{child.GetType().Name}");
                ClearChild(child);
            }
            ...
            foreach (Transform child in above3DLayer)
            {
                Log.Print($"HandleUI RmvUI ClearSceneAll() ui above3DLayer ___ name:{child.name}, child GetType().Name:{child.GetType().Name}");
                ClearChild(child);
            }
